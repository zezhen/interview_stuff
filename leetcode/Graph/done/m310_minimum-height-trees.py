...
class Solution(object):
    # this solution is a variance of topological sort
    ...
    def findMinHeightTrees_dfs(self, n, edges):
        ...
        def bfs_depth(queue, node, n, minDepth):
            queue.append((node, 0))
            visited = [False] * n
            maxDepth = 0
            while len(queue) > 0:
        ...
        for i in xrange(n):
            depth = bfs_depth(queue, i, n, minDepth)
            if depth < minDepth:
                ans = []
                ans.append(i)
                minDepth = depth
