...
class Solution(object):
    # memo solution exceed the time limitation
    # my dp has some logic problem
    # refer to java implementation below
    ...
    def maxProfit(self, k, prices):
        ...
        dp = [[0 for _ in xrange(k+1) ] for _ in xrange(len(prices)+1) ]

        dp[2][1] = max(0, prices[1] - prices[0])
        for i in xrange(2, len(prices)+1):
            for j in xrange(1,min(i, k+1)):
                donothing1 = dp[i][j-1]
                donothing2 = dp[i-1][j-1]
                onemore = max(map(lambda m: dp[m][j-1] + max(0, prices[i-1] - prices[m]), range(i-1)))
                # onemore = (dp[i-1][j-1] + max(0, prices[i-1] - prices[i-2])) if i > 0 and j > 0 else 0
                dp[i][j] = max(donothing1, donothing2, onemore)
        return dp[len(prices)][k]

    def maxProfit_memo(self, k, prices):
        """
        :type k: int
        :type prices: List[int]
        :rtype: int
        ...
        def trade(i, c):
            ...
            if i >= len(prices) - 1 or c == 0: return 0

            maxProfit, minV = 0, prices[i]
            for j in xrange(i+1, len(prices)):
                profit = max(0, prices[j] - minV) + trade(j+1, c-1)
                minV = min(minV, prices[j])
                maxProfit = max(maxProfit, profit)
                # print i,j,c,profit,maxProfit
...
public int maxProfit(int k, int[] prices) {
    ...
    for (int i = 1; i <= k; i++) {
        ...
        for (int j = 1; j < len; j++) {
            ...
            # tmpMax is the key, my solution looks back again, which is not necessary
            # the for loop order is also very important, i choose str first, then k
...
assert s.maxProfit(0,[3,3,5,0,0,3,1,4]) == 0
assert s.maxProfit(1,[3,3,5,0,0,3,1,4]) == 4
